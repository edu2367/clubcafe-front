...
export default function Home() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center relative overflow-hidden bg-black font-sans transition-all duration-1000">
      ...
      {/* CONTENIDO PRINCIPAL */}
      <div className="relative z-10 w-full max-w-3xl flex flex-col items-center justify-center min-h-[550px]">

        {/* --- ESCENA 1: MARCA Y BOTONES (DESAPARECE EN PET FRIENDLY) --- */}
        <div className={`absolute inset-0 flex flex-col items-center justify-center transition-all duration-1000 transform ${isPetFriendlyView ? 'opacity-0 translate-y-10 scale-95 pointer-events-none' : 'opacity-100 translate-y-0 scale-100'}`}>
          ...
          {/* CONTENEDOR DE BOTONES EN VERTICAL */}
          <div className="flex flex-col items-center gap-6">
            ...
            {/* BOTÓN CARTA: ABAJO, MONOCROMÁTICO Y ELEGANTE */}
            <a
              href="https://app.qrpro.io/q/3845c8"
              target="_blank"
              rel="noopener noreferrer"
              className="group inline-flex items-center gap-3 px-8 py-3 border border-white/20 rounded-full text-white hover:bg-white/10 hover:border-white/50 transition-all shadow-2xl"
            >
              <span className="text-[10px] font-bold uppercase tracking-[0.3em]">Ver Nuestra Carta</span>
              <ChevronDown size={14} className="text-white/50 group-hover:translate-y-1 transition-transform" />
            </a>
          </div>
        </div>
      ...
      <div className="absolute bottom-6 opacity-40 hover:opacity-100 transition-opacity">
        <div className="flex items-center gap-6">
          <a href="https://www.instagram.com/clubcafe.temuco" target="_blank" rel="noopener noreferrer" className="text-white hover:scale-110 transition-transform">
            <Instagram size={18} />
          </a>
          <a href="mailto:hola@clubcafe.cl" className="text-white hover:scale-110 transition-transform">
            <Mail size={18} />
          </a>
        </div>
      </div>
